...
def chiSquaredTest (observed : Array (Array Nat)) : Float × Nat :=
  ...
  let colSums := Id.run do
    let mut cs := Array.replicate cols 0
...
def crossTab (xs ys : Array String) : Array String × Array String × Array (Array Nat) :=
  ...
  let mat := Id.run do
    let mut m := Array.replicate rowLabels.size (Array.replicate colLabels.size 0)
    for idx in [:xs.size] do
      let x := xs[idx]!
      let y := ys[idx]!
      match rowLabels.findIdx? (· == x), colLabels.findIdx? (· == y) with
      | some ri, some ci =>
        let row := m[ri]!
        m := m.set! ri (row.set! ci (row[ci]! + 1))
      | _, _ => pure ()
    m
