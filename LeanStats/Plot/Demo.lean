...
<div class='controls'>
  <label>Effective df: <span id='dfVal'>5</span></label>
...
function update(){
  const sliderVal=+document.getElementById('dfSlider').value;
  const bw=(xMax-xMin)/sliderVal;
  const smooth=kernelSmooth(xs,ys,xGrid,bw);
  const smoothAtX=kernelSmooth(xs,ys,xs,bw);
  const resids=ys.map((y,i)=>y-smoothAtX[i]);
  // Compute actual effective df = trace(smoother matrix S)
  // S[i,j] = K((xi-xj)/bw) / sum_k K((xi-xk)/bw)
  // trace(S) = sum_i S[i,i] = sum_i K(0) / sum_k K((xi-xk)/bw)
  let traceS=0;
  for(let i=0;i<xs.length;i++){
    let den=0;
    for(let j=0;j<xs.length;j++) den+=Math.exp(-0.5*((xs[i]-xs[j])/bw)**2);
    traceS+=1.0/den;  // K(0)=1 for standard Gaussian
  }
  document.getElementById('dfVal').textContent=traceS.toFixed(1);
  // Scatter + Smooth
...
// Homage to Bob Stine: initialize at effective df = 5.0
// (Bob always picks 5 degrees of freedom for a kernel smoother)
(function initBobsDf(){
  const sl=document.getElementById('dfSlider');
  let best=5,bestDiff=999;
  for(let v=2;v<=40;v++){
    const bw=(xMax-xMin)/v;
    let tr=0;
    for(let i=0;i<xs.length;i++){
      let den=0;
      for(let j=0;j<xs.length;j++) den+=Math.exp(-0.5*((xs[i]-xs[j])/bw)**2);
    ...
    if(Math.abs(tr-5.0)<bestDiff){bestDiff=Math.abs(tr-5.0);best=v;}
  }
  sl.value=best;
})();
update();
