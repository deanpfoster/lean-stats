...
private def dashJs2 : String :=
  "function drawPanel(pi){" ++
    "var p=panels[pi];var svg=document.getElementById(p.svgId);if(!svg)return;" ++
    "if(p.type==='scatter')drawScatterPanel(pi);" ++
    "else if(p.type==='histogram')drawHistoPanel(pi);" ++
    "else if(p.type==='avplot')drawAvPanel(pi);" ++
    "else if(p.type==='residual')drawResidPanel(pi);" ++
  "}" ++
  "function drawAll(){for(var i=0;i<panels.length;i++)drawPanel(i);drawLegend()}" ++
  "function drawScatterPanel(pi){" ++
    "var p=panels[pi];var svg=document.getElementById(p.svgId);" ++
    "var xVar=p.config.x||numVars[0];var xf=p.config.xform||'linear';var yf=p.config.yform||'linear';" ++
    "var deg=p.config.degree||1;var xArr=data.xs[xVar]||[];" ++
    "var pts=[];for(var i=0;i<n;i++){if(pointState[i].hidden)continue;var xt=tx(xArr[i],xf),yt=tx(data.y[i],yf);if(!isNaN(xt)&&isFinite(xt)&&!isNaN(yt)&&isFinite(yt))pts.push({idx:i,x:xt,y:yt})}" ++
    ...
    "});" ++
    "svg.innerHTML=s;bindPanel(pi)" ++
  "}"

private def dashJs3 : String :=
  "function drawHistoPanel(pi){" ++
    "var p=panels[pi];var svg=document.getElementById(p.svgId);" ++
    "var varName=p.config.var||grpVars[0]||'';var hM={t:10,r:10,b:25,l:35};var hpw=PW-hM.l-hM.r,hph=PH-hM.t-hM.b;" ++
    "var s=\"<rect x='0' y='0' width='\"+PW+\"' height='\"+PH+\"' fill='transparent' class='bg' data-pi='\"+pi+\"'/>\";" ++
    "if(data.groups[varName]){" ++
      "var gs=data.groups[varName];var gNames=[];gs.forEach(function(g){if(gNames.indexOf(g)<0)gNames.push(g)});" ++
      "var barH=Math.min(35,hph/gNames.length-3);" ++
      "var counts=gNames.map(function(g){var c=0;for(var i=0;i<n;i++)if(gs[i]===g&&!pointState[i].hidden)c++;return c});" ++
      "var maxC=Math.max.apply(null,counts)||1;" ++
      "gNames.forEach(function(g,gi){" ++
        "var y=hM.t+gi*(barH+3);var w=counts[gi]/maxC*hpw;" ++
        "var anyS=false,anyE=false;for(var i=0;i<n;i++)if(gs[i]===g){if(pointState[i].selected)anyS=true;if(pointState[i].excluded)anyE=true}" ++
      ...
      "});" ++
    "}else if(data.xs[varName]){" ++
      "var vals=data.xs[varName];var vis=[];for(var i=0;i<n;i++)if(!pointState[i].hidden)vis.push(vals[i]);" ++
      ...
      "for(var i=0;i<n;i++){if(pointState[i].hidden)continue;var v=vals[i];var b=Math.min(9,Math.floor((v-vMin)/vR*10));bins[b]++;binIdx[i]=b}" ++
      "var maxB=Math.max.apply(null,bins)||1;var barH=hph/10-2;" ++
      "for(var b=0;b<10;b++){" ++
        "var y=hM.t+b*(barH+2);var w=bins[b]/maxB*hpw;" ++
        "var anyS=false;for(var i=0;i<n;i++)if(binIdx[i]===b&&pointState[i].selected)anyS=true;" ++
    ...
    "svg.innerHTML=s;bindPanel(pi)" ++
  "}" ++
  "function drawAvPanel(pi){" ++
    "var p=panels[pi];var svg=document.getElementById(p.svgId);" ++
    "var pred=p.config.predictor||numVars[0];var others=numVars.filter(function(v){return v!==pred});" ++
    "var idx=[];for(var i=0;i<n;i++)if(!pointState[i].excluded&&!pointState[i].hidden)idx.push(i);" ++
    ...
    -- residualize y on others, residualize pred on others
    "var X=idx.map(function(i){var row=[1];others.forEach(function(v){row.push(data.xs[v][i])});return row});" ++
    "var yv=idx.map(function(i){return data.y[i]});var xv=idx.map(function(i){return data.xs[pred][i]});" ++
  ...
  "function drawResidPanel(pi){" ++
    ...
    "var X=idx.map(function(i){var row=[1];numVars.forEach(function(v){row.push(data.xs[v][i])});return row});" ++
    "var yv=idx.map(function(i){return data.y[i]});" ++
...
private def dashJs4 : String :=
  "function bindPanel(pi){" ++
    "var svg=document.getElementById(panels[pi].svgId);" ++
    "svg.querySelectorAll('[data-idx]').forEach(function(el){el.addEventListener('click',function(e){e.stopPropagation();var idx=parseInt(el.dataset.idx);if(e.shiftKey){pointState[idx].selected=!pointState[idx].selected}else{for(var i=0;i<n;i++)pointState[i].selected=false;pointState[idx].selected=true}drawAll();report('selection_changed')})});" ++
    "svg.querySelectorAll('.bg').forEach(function(el){el.addEventListener('click',function(e){if(e.target===el){for(var i=0;i<n;i++)pointState[i].selected=false;drawAll();report('selection_changed')}})});" ++
    "svg.querySelectorAll('[data-grp]').forEach(function(el){el.addEventListener('click',function(e){e.stopPropagation();var g=el.dataset.grp;var varName=panels[pi].config.var||grpVars[0];" ++
      "if(!e.shiftKey){for(var i=0;i<n;i++)pointState[i].selected=false}" ++
      "var gs=data.groups[varName];if(gs)for(var i=0;i<n;i++)if(gs[i]===g)pointState[i].selected=true;" ++
    ...
    "svg.querySelectorAll('[data-bin]').forEach(function(el){el.addEventListener('click',function(e){e.stopPropagation();var b=parseInt(el.dataset.bin);var varName=el.dataset.var;" ++
      "var vals=data.xs[varName];if(!vals)return;" ++
      "var vis=[];for(var i=0;i<n;i++)if(!pointState[i].hidden)vis.push(vals[i]);" ++
      "var vMin=Math.min.apply(null,vis),vMax=Math.max.apply(null,vis),vR=vMax-vMin||1;" ++
      "if(!e.shiftKey){for(var i=0;i<n;i++)pointState[i].selected=false}" ++
      "for(var i=0;i<n;i++){if(pointState[i].hidden)continue;var bi=Math.min(9,Math.floor((vals[i]-vMin)/vR*10));if(bi===b)pointState[i].selected=true}" ++
      "drawAll();report('selection_changed')})});" ++
  "}" ++
  "function addPanel(type,config){" ++
    "var id='panel_'+panels.length;var idx=panels.length;" ++
    "panels.push({type:type,config:config||{},svgId:id});" ++
    ...
    "var svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.id=id;svg.setAttribute('width',PW);svg.setAttribute('height',PH);" ++
    "var ctrl=document.createElement('div');ctrl.className='panel-ctrl';" ++
    "if(type==='scatter'){" ++
      "var sel='<select onchange=\"panels['+idx+'].config.x=this.value;drawPanel('+idx+')\">';numVars.forEach(function(v){sel+='<option'+(v===(config&&config.x||numVars[0])?' selected':'')+'>'+v+'</option>'});sel+='</select> ';" ++
      "sel+='<select onchange=\"panels['+idx+'].config.xform=this.value;drawPanel('+idx+')\"><option value=\"linear\">lin</option><option value=\"log\">log</option><option value=\"sqrt\">√</option><option value=\"recip\">1/x</option><option value=\"square\">x²</option></select> ';" ++
      "sel+='deg<select onchange=\"panels['+idx+'].config.degree=+this.value;drawPanel('+idx+')\"><option>1</option><option>2</option><option>3</option><option>4</option><option>5</option></select>';" ++
      "ctrl.innerHTML=sel" ++
    "}else if(type==='histogram'){" ++
      "var sel='<select onchange=\"panels['+idx+'].config.var=this.value;drawPanel('+idx+')\">';grpVars.concat(numVars).forEach(function(v){sel+='<option'+(v===(config&&config.var)?' selected':'')+'>'+v+'</option>'});sel+='</select>';" ++
      "ctrl.innerHTML=sel" ++
    "}" ++
  ...
  "function report(event,extra){" ++
    "var sel=[],exc=[];var nv=0;for(var i=0;i<n;i++){if(pointState[i].selected)sel.push(i);if(pointState[i].excluded)exc.push(i);if(!pointState[i].hidden)nv++}" ++
  ...
  "function drawLegend(){" ++
    ...
    "fits.forEach(function(spec,idx){var col=colors[idx%colors.length];h+='<span style=\"display:inline-block;width:18px;height:3px;background:'+col+';vertical-align:middle;margin-right:4px\"></span>Chk'+(idx+1)+' '});" ++
    "el.innerHTML=h" ++
  "}"
...
private def dashJs5 : String :=
  ...
  "function handleCommand(msg){" ++
    "switch(msg.cmd){" ++
      "case'addVariable':if(msg.type==='group'){data.groups[msg.name]=msg.values;grpVars.push(msg.name)}else{data.xs[msg.name]=msg.values;numVars.push(msg.name)}break;" ++
      "case'addPanel':addPanel(msg.type,msg);break;" ++
      "case'removePanel':if(msg.id<panels.length){var el=document.getElementById(panels[msg.id].svgId);if(el&&el.parentNode)el.parentNode.parentNode.removeChild(el.parentNode);panels[msg.id]={type:'removed',config:{},svgId:''}}break;" ++
      "case'select':for(var i=0;i<n;i++)pointState[i].selected=false;(msg.ids||[]).forEach(function(id){if(id<n)pointState[id].selected=true});break;" ++
  ...
  "function doCheckpoint(){" ++
    "panels.forEach(function(p,pi){" ++
      ...
      "var xArr=data.xs[xVar]||[];var fp=[];" ++
      "for(var i=0;i<n;i++){if(pointState[i].excluded||pointState[i].hidden)continue;var xt=tx(xArr[i],xf),yt=tx(data.y[i],p.config.yform||'linear');if(!isNaN(xt)&&isFinite(xt)&&!isNaN(yt)&&isFinite(yt))fp.push({x:xt,y:yt})}" ++
      "if(fp.length<=deg)return;" ++
      "var coef=polyFit(fp.map(function(q){return q.x}),fp.map(function(q){return q.y}),deg);" ++
      "if(coef)fits.push({coef:coef,panelIdx:pi})" ++
    "});" ++
    "drawAll();report('checkpoint',{panels:panels.length,fits:fits.length})" ++
  "}" ++
  -- Toolbar
  "document.getElementById('chkBtn').addEventListener('click',doCheckpoint);" ++
  "document.getElementById('clearBtn').addEventListener('click',function(){fits=[];drawAll()});" ++
  "document.getElementById('invertBtn').addEventListener('click',function(){for(var i=0;i<n;i++)if(!pointState[i].hidden)pointState[i].selected=!pointState[i].selected;drawAll();report('selection_changed')});" ++
  "document.getElementById('keepBtn').addEventListener('click',function(){" ++
    "var state={event:'keep',pointState:pointState,fits:fits,panels:panels};" ++
